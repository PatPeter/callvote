...
namespace callvote.Commands
{
    ...
    class Number10Command : ICommand
    {
        public string Command => "10";

        public string[] Aliases => new string[] { };

        public string Description => Plugin.Instance.Config.Localizations.DescriptionNumber10;
		...
		public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
        {
			...
			if (sender is PlayerCommandSender)
            {
                ...
                {
                    response = Plugin.Instance.VoteHandler(player, 10);
                }
                else
                {
